...
class InferencePipeline:
    def __init__(
        ...
        with self.device:
            ...
            ss_generator = self.init_ss_generator(ss_generator_config_path, ss_generator_ckpt_path)
            ...
            ss_decoder = self.init_ss_decoder(ss_decoder_config_path, ss_decoder_ckpt_path)
            ss_encoder = self.init_ss_encoder(ss_encoder_config_path, ss_encoder_ckpt_path)

            if not self.stage1_only:
                if 'gaussian' in self.decode_formats or 'gaussian_4' in self.decode_formats:
                ...
                if 'mesh' in self.decode_formats:
    ...
    def init_ss_preprocessor(self, ss_preprocessor, ss_generator_config_path):
        ...
        config = OmegaConf.load(os.path.join(self.workspace_dir, ss_generator_config_path))['tdfy'][
            'val_preprocessor'
        ]
        return instantiate(config)

    def init_ss_generator(self, ss_generator_config_path, ss_generator_ckpt_path):
        config = OmegaConf.load(os.path.join(self.workspace_dir, ss_generator_config_path))[
            'module'
        ]['generator']['backbone']
    ...
    def init_slat_generator(self, slat_generator_config_path, slat_generator_ckpt_path):
        config = OmegaConf.load(os.path.join(self.workspace_dir, slat_generator_config_path))[
            'module'
        ]['generator']['backbone']
        state_dict_prefix_func = filter_and_remove_prefix_state_dict_fn('_base_models.generator.')
        return self.instantiate_and_load_from_pretrained(
    ...
    def init_ss_encoder(self, ss_encoder_config_path, ss_encoder_ckpt_path):
        if ss_encoder_ckpt_path is not None:
            ...
            config = OmegaConf.load(os.path.join(self.workspace_dir, ss_encoder_config_path))
    ...
    def init_ss_decoder(self, ss_decoder_config_path, ss_decoder_ckpt_path):
        # override to avoid problem loading
        config = OmegaConf.load(os.path.join(self.workspace_dir, ss_decoder_config_path))
        if 'pretrained_ckpt_path' in config:
            del config['pretrained_ckpt_path']
        return self.instantiate_and_load_from_pretrained(
            config,
    ...
    def init_slat_decoder_gs(self, slat_decoder_gs_config_path, slat_decoder_gs_ckpt_path):
        if slat_decoder_gs_config_path is None:
            return None
        else:
            return self.instantiate_and_load_from_pretrained(
                OmegaConf.load(os.path.join(self.workspace_dir, slat_decoder_gs_config_path)),
                os.path.join(self.workspace_dir, slat_decoder_gs_ckpt_path),
                device=self.device,
                state_dict_key=None,
            )

    def init_slat_decoder_mesh(self, slat_decoder_mesh_config_path, slat_decoder_mesh_ckpt_path):
        return self.instantiate_and_load_from_pretrained(
            OmegaConf.load(os.path.join(self.workspace_dir, slat_decoder_mesh_config_path)),
            os.path.join(self.workspace_dir, slat_decoder_mesh_ckpt_path),
            device=self.device,
            state_dict_key=None,
        )

    def init_ss_condition_embedder(self, ss_generator_config_path, ss_generator_ckpt_path):
        conf = OmegaConf.load(os.path.join(self.workspace_dir, ss_generator_config_path))
    ...
    def init_slat_condition_embedder(self, slat_generator_config_path, slat_generator_ckpt_path):
        return self.init_ss_condition_embedder(slat_generator_config_path, slat_generator_ckpt_path)
    ...
    def postprocess_slat_output(
        ...
        if 'mesh' in outputs:
            # If we don't have gaussians, we can't bake textures
            if 'gaussian' not in outputs:
                if with_texture_baking:
                    logger.warning(
                        'Texture baking requested but Gaussian representation is missing. Disabling texture baking.'
                    )
                    with_texture_baking = False
                app_rep = None
            else:
                app_rep = outputs['gaussian'][0]
    ...
    def get_condition_input(self, condition_embedder, input_dict, input_mapping):
        ...
        condition_kwargs = {k: v for k, v in input_dict.items() if k not in input_mapping}
    ...
    def preprocess_image(self, image: Union[Image.Image, np.ndarray], preprocessor) -> torch.Tensor:
        ...
        processed_rgb_image = self._apply_transform(processed_rgb_image, preprocessor.img_transform)
        processed_mask = self._apply_transform(processed_mask, preprocessor.mask_transform)

        # full image, with only processing from the image
        rgb_image = self._apply_transform(rgb_image, preprocessor.img_transform)
        rgb_image_mask = self._apply_transform(rgb_image_mask, preprocessor.mask_transform)
        item = {
