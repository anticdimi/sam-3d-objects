...
class InferencePipelinePointMap(InferencePipeline):
    ...
    def _compile(self):
        ...
        for embedder, _ in self.condition_embedders['ss_condition_embedder'].embedder_list:
    ...
    def _preprocess_image_and_mask_pointmap(
        ...
        for trans in img_mask_pointmap_joint_transform:
            rgb_image, mask_image, pointmap = trans(rgb_image, mask_image, pointmap=pointmap)
    ...
    def _clip_pointmap(
        ...
        mask_resized = torchvision.transforms.functional.resize(
            mask, pointmap_size, interpolation=torchvision.transforms.InterpolationMode.NEAREST
        ).squeeze(0)
    ...
    def estimate_plane(self, pointmap_dict, image, ground_area_threshold=0.25, min_points=100):
        ...
        floor_mask = (
            type(self)._down_sample_img(torch.from_numpy(image[..., -1]).float().unsqueeze(0))[0]
            > 0.5
        )
