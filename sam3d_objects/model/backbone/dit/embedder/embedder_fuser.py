...
class EmbedderFuser(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        for i, (condition_embedder, kwargs_info) in enumerate(self.embedder_list):
            supports_packed = condition_embedder.supports_packed_batch

            if supports_packed and len(kwargs_info) > 1:
                input_conds = []
                for kwarg_name, _pos_group in kwargs_info:
                    if kwarg_name not in kwargs:
                        logger.warning(f'{kwarg_name} not in kwargs to condition embedder!')
                    input_conds.append(kwargs[kwarg_name])

                cond_tokens = condition_embedder(input_conds)
                assert isinstance(cond_tokens, (list, tuple)), (
                    'Packed embedder must return a list/tuple of tokens'
                )
                assert len(cond_tokens) == len(kwargs_info), (
                    'Packed embedder must return one token tensor per input'
                )

                for cond_token, (kwarg_name, pos_group) in zip(cond_tokens, kwargs_info):
