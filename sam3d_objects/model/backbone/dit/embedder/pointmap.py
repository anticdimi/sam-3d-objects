...
class PointPatchEmbed(nn.Module):
    ...
    def forward(
        self,
        xyz: torch.Tensor | list[torch.Tensor] | tuple[torch.Tensor, ...],
        valid_mask: torch.Tensor | list[torch.Tensor] | tuple[torch.Tensor, ...] | None = None,
    ):
        """
        ...
        if isinstance(xyz, (list, tuple)):
            assert len(xyz) > 0, 'Expected non-empty list/tuple of pointmaps'
            assert all(isinstance(v, torch.Tensor) for v in xyz), (
                'All packed pointmaps must be tensors'
            )
            assert all(v.dim() == 4 for v in xyz), 'Packed pointmaps must be [B,C,H,W] tensors'
            assert all(v.dtype == xyz[0].dtype for v in xyz), 'Packed pointmaps must share dtype'
            assert all(v.device == xyz[0].device for v in xyz), 'Packed pointmaps must share device'

            if valid_mask is None:
                valid_masks = [None] * len(xyz)
            else:
                assert isinstance(valid_mask, (list, tuple)), (
                    'When xyz is packed, valid_mask must be None or list/tuple'
                )
                assert len(valid_mask) == len(xyz), 'valid_mask list must match xyz list length'
                valid_masks = list(valid_mask)

            xs = []
            batch_sizes = []
            H = W = None
            for xyz_i, vm_i in zip(xyz, valid_masks):
                x_i, B_i, H_i, W_i = self.embed_pointmap_windows(xyz_i, vm_i)
                xs.append(x_i)
                batch_sizes.append(int(B_i))
                if H is None:
                    H, W = int(H_i), int(W_i)
                else:
                    assert int(H_i) == int(H) and int(W_i) == int(W), (
                        'Packed pointmaps must resize to the same HxW'
                    )

            x_packed = torch.cat(xs, dim=0)
            out_packed = self.inner_forward(x_packed, sum(batch_sizes), int(H), int(W))
            return list(torch.split(out_packed, batch_sizes, dim=0))
